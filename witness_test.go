...
//	ctx = New(ctx, traceID, WithZapLogger(logger), WithRecords(record.String("service", "test-service")))
//
...
//	ErrorNetwork(ctx, "test-error", record.String("test-key", "test-value"))
//	Debug(ctx, "test-error", record.String("test-key", "test-value"))
//
